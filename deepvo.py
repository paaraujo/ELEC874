...
def train(model, train_loader, optimizer, device, epoch, weight_folder):
    ...
    for seq, pos, ang in train_loader:
        ...
        loss.backward()
        optimizer.step()
        print('Batches processed: {} / {}'.format(i, len(train_loader)), end='\r')
    train_losses /= len(train_loader)
    total_time = dt.datetime.now() - start
    print(f"Train Epoch {epoch}th in {total_time} | Loss: {train_losses}")
...
def validate(model, test_loader, optimizer, device, epoch):
    ...
    total_time = dt.datetime.now() - start
    print(f"Validate Epoch {epoch}th in {total_time} | Loss: {test_loss}")
    return test_loss
...
if __name__ == '__main__':
    ...
    VAL_SEQ = ["99"]
    TEST_SEQ = ["99"]
    PREV_WEIGHTS_PATH = 'flownets_EPE1.951.pth.tar'
    PREV_WEIGHTS_ORIGIN = 'FlowNet'
    KITTI_PATH = 'kitti'
    WEIGHTS_PATH = 'opt6'
    ...
    # dataloaders
    train_dl = DataLoader(KittiDataset(TRAIN_SEQ, SEQ_LEN, KITTI_PATH, transform=transform, normalizer=normalizer, left=True), batch_size=8, num_workers=8, pin_memory=True, drop_last=True, shuffle=True)
    val_dl = DataLoader(KittiValidationDataset(VAL_SEQ, SEQ_LEN, VAL_LEN, KITTI_PATH, transform=transform, normalizer=normalizer, left=True), batch_size=8, num_workers=8, drop_last=True, shuffle=False)
    ...
    for e in range(cur + 1, cur + EPOCH + 1):
        train_losses.append(train(model, train_dl, optimizer, device, e, WEIGHTS_PATH))
        #valid_losses.append(validate(model, val_dl, optimizer, device, e))
    ...
    # testing
    test_dl = DataLoader(KittiDataset([TEST_SEQ[0]], SEQ_LEN, KITTI_PATH, transform=transform, normalizer=normalizer, left=True),batch_size=8, num_workers=8, pin_memory=True, drop_last=True, shuffle=False)
    gt,estimated = test(model, test_dl, device, KITTI_PATH, test_seq=TEST_SEQ[0])
    gt = np.array(gt)
    estimated = np.array(estimated)

    fig, (ax1, ax2, ax3) = plt.subplots(1, 3)
    fig.suptitle('Attitude angles')
    ax1.plot(gt[:,0],label='GT')
    ax1.plot(estimated[:,0],label='EST')
    ax1.legend()
    ax2.plot(gt[:,1],label='GT')
    ax2.plot(estimated[:,1],label='EST')
    ax2.legend()
    ax3.plot(gt[:,2],label='GT')
    ax3.plot(estimated[:,2],label='EST')
    ax3.legend()
    plt.legend()
    plt.show()
    
    draw_route(gt,estimated,TEST_SEQ[0],WEIGHTS_PATH)
